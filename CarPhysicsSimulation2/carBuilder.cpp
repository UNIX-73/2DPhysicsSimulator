...
CarBuilder::CarBuilder(SimulationManager& simManager, double vAxisLength, double hAxisLength) :
    ...
    //  Engine
    std::shared_ptr<RigidConstraint> engineToVAxis = std::make_shared<RigidConstraint>(Constraint(engine, vAxis), 0.0, 1000.0, true, V2(-vAxisLength * 0.12, 0.0));
    simManager.AddConstraint(engineToVAxis);

  

    //  FAxis
    std::shared_ptr<RigidConstraint> fAxisToVAxisLength = std::make_shared<RigidConstraint>(Constraint(vAxis, fAxis), 0.0, 1000.0, true, V2(-vAxisLength / 2.0, 0.0));
    simManager.AddConstraint(fAxisToVAxisLength);
    


    //  RAxis
    std::shared_ptr<RigidConstraint> rAxisToVAxisLength = std::make_shared<RigidConstraint>(Constraint(vAxis, rAxis), 0.0, 1000.0, true, V2(vAxisLength / 2.0, 0.0));
